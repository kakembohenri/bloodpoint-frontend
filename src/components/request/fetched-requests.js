...
const columns = [
  ...
  { id: "ordered_by", label: "Ordered\u00a0By", minWidth: 100 },
...
export default function Requests({ request }) {
  ...
  const [filteredRows, setFilteredRows] = useState(request);
  ...
  const handleDelete = (id) => {
    console.log(id);
    axios
      .delete(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/donor/${id}`)
      .then((response) => {
            ...
            <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <Box sx={{ width: "100%" }}>
                <TextField
                  ...
                  placeholder="Search for blood requests"
                  variant="outlined"
                />
              </Box>
            </Box>
